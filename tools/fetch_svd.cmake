    ...
    if(${MATCH_WEIGHT_MAX} GREATER 0)
        if(NOT EXISTS ${CMAKE_CURRENT_SOURCE_DIR}/build/_deps/SVD/${SVD_FILE})
            ...
            if (NOT ${REPO_DL_ERROR} STREQUAL "\"No error\"")
                ...
                file(SIZE ${CMAKE_CURRENT_SOURCE_DIR}/build/_deps/SVD/${SVD_FILE} SVD_SIZE)
                if(SVD_SIZE EQUAL 0)
                    message(WARNING "-- Search for .svd files: Downloaded ${SVD_FILE} is empty. Removing file and skipping.")
                    file(REMOVE ${CMAKE_CURRENT_SOURCE_DIR}/build/_deps/SVD/${SVD_FILE})
                endif()
            endif()
        endif()
        if(EXISTS ${CMAKE_CURRENT_SOURCE_DIR}/build/_deps/SVD/${SVD_FILE})
            set(ENV{BEST_FIT_SVD} /build/_deps/SVD/${SVD_FILE})
            message("-- Search for .svd files: ${MCU} done. Relative path: $ENV{BEST_FIT_SVD}")
        else()
            message(WARNING "-- Search for .svd files: ${SVD_FILE} not available after download attempts")
        endif()
    else()
