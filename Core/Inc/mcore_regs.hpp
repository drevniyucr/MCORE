...
namespace SCB {
    static constexpr uint32_t SCB_BASE = 0xE000ED00UL;

    // ---------------------------
    // CPUID – CPU Identification Register
    // ---------------------------
    using CPUID_Reg = Register<SCB_BASE + 0x00, ReadOnly>;
    struct _CPUID {
        using IMPLEMENTER = Field<CPUID_Reg, 24, 8>;  // Bits [31:24]
        using VARIANT     = Field<CPUID_Reg, 20, 4>;  // Bits [23:20]
        using ARCHITECTURE= Field<CPUID_Reg, 16, 4>;
        using PARTNO      = Field<CPUID_Reg, 4, 12>;  // Bits [15:4]
        using REVISION    = Field<CPUID_Reg, 0, 4>;   // Bits [3:0]
    };

    // ---------------------------
    // ICSR – Interrupt Control and State Register
    // ---------------------------
    using ICSR_Reg = Register<SCB_BASE + 0x04, ReadWrite>;
    struct _ICSR {
        using NMIPENDSET  = Field<ICSR_Reg, 31, 1>;
        using PENDSVSET   = Field<ICSR_Reg, 28, 1>;
        using PENDSVCLR   = Field<ICSR_Reg, 27, 1>;
        using PENDSTSET   = Field<ICSR_Reg, 26, 1>;
        using PENDSTCLR   = Field<ICSR_Reg, 25, 1>;
        using ISRPREEMPT  = Field<ICSR_Reg, 23, 1>;
        using ISRPENDING  = Field<ICSR_Reg, 22, 1>;
        using VECTPENDING = Field<ICSR_Reg, 12, 9>;
        using RETTOBASE   = Field<ICSR_Reg, 11, 1>;
        using VECTACTIVE  = Field<ICSR_Reg, 0, 9>;
    };

    // ---------------------------
    // VTOR – Vector Table Offset Register
    // ---------------------------
    using VTOR_Reg = Register<SCB_BASE + 0x08, ReadWrite>;
    struct _VTOR {
        using TBLOFF = Field<VTOR_Reg, 7, 25>; // Vector table base offset
    };

    // ---------------------------
    // AIRCR – Application Interrupt and Reset Control Register
    // ---------------------------
    using AIRCR_Reg = Register<SCB_BASE + 0x0C, ReadWrite>;
    struct _AIRCR {
        using VECTKEY    = Field<AIRCR_Reg, 16, 16>; // Write key (0x05FA)
        using ENDIANNESS = Field<AIRCR_Reg, 15, 1>;
        using PRIGROUP   = Field<AIRCR_Reg, 8, 3>;
        using SYSRESETREQ= Field<AIRCR_Reg, 2, 1>;
        using VECTCLRACTIVE = Field<AIRCR_Reg, 1, 1>;
        using VECTRESET  = Field<AIRCR_Reg, 0, 1>;
    };

    // ---------------------------
    // SCR – System Control Register
    // ---------------------------
    using SCR_Reg = Register<SCB_BASE + 0x10, ReadWrite>;
    struct _SCR {
        using SEVONPEND = Field<SCR_Reg, 4, 1>;
        using SLEEPDEEP = Field<SCR_Reg, 2, 1>;
        using SLEEPONEXIT = Field<SCR_Reg, 1, 1>;
    };

    // ---------------------------
    // CCR – Configuration and Control Register
    // ---------------------------
    using CCR_Reg = Register<SCB_BASE + 0x14, ReadWrite>;
    struct _CCR {
        using BP         = Field<CCR_Reg, 18, 1>;
        using IC         = Field<CCR_Reg, 17, 1>;
        using DC         = Field<CCR_Reg, 16, 1>;
        using STKALIGN   = Field<CCR_Reg, 9, 1>;
        using BFHFNMIGN  = Field<CCR_Reg, 8, 1>;
        using DIV_0_TRP  = Field<CCR_Reg, 4, 1>;
        using UNALIGN_TRP= Field<CCR_Reg, 3, 1>;
        using USERSETMPEND = Field<CCR_Reg, 1, 1>;
        using NONBASETHRDENA = Field<CCR_Reg, 0, 1>;
    };

    // ---------------------------
    // SHP[0..11] – System Handler Priority Registers
    // ---------------------------
    using SHPR1_Reg = Register<SCB_BASE + 0x18, ReadWrite>;
    using SHPR2_Reg = Register<SCB_BASE + 0x1C, ReadWrite>;
    using SHPR3_Reg = Register<SCB_BASE + 0x20, ReadWrite>;
    // можно также оформить _SHPR1, _SHPR2, _SHPR3, если нужно обращаться к отдельным байтам

    // ---------------------------
    // SHCSR – System Handler Control and State Register
    // ---------------------------
    using SHCSR_Reg = Register<SCB_BASE + 0x24, ReadWrite>;
    struct _SHCSR {
        using USGFAULTENA = Field<SHCSR_Reg, 18, 1>;
        using BUSFAULTENA = Field<SHCSR_Reg, 17, 1>;
        using MEMFAULTENA = Field<SHCSR_Reg, 16, 1>;
        using SVCALLPENDED = Field<SHCSR_Reg, 15, 1>;
        using BUSFAULTPENDED = Field<SHCSR_Reg, 14, 1>;
        using MEMFAULTPENDED = Field<SHCSR_Reg, 13, 1>;
        using USGFAULTPENDED = Field<SHCSR_Reg, 12, 1>;
        using SYSTICKACT = Field<SHCSR_Reg, 11, 1>;
        using PENDSVACT = Field<SHCSR_Reg, 10, 1>;
        using MONITORACT = Field<SHCSR_Reg, 8, 1>;
        using SVCALLACT = Field<SHCSR_Reg, 7, 1>;
        using USGFAULTACT = Field<SHCSR_Reg, 3, 1>;
        using BUSFAULTACT = Field<SHCSR_Reg, 1, 1>;
        using MEMFAULTACT = Field<SHCSR_Reg, 0, 1>;
    };

    // ---------------------------
    // CFSR – Configurable Fault Status Register
    // ---------------------------
    using CFSR_Reg = Register<SCB_BASE + 0x28, ReadWrite>;
    struct _CFSR {
        using UFSR = Field<CFSR_Reg, 16, 16>;
...
namespace MPU {
    static constexpr uint32_t MPU_BASE = 0xE000ED90;

    // MPU Type Register
    using TYPE_Reg = Register<MPU_BASE + 0x0, ReadOnly>;
    struct _TYPE {
         using IREGION = Field<TYPE_Reg, 16, 8>; // Instruction region
         using DREGION = Field<TYPE_Reg, 8, 8>; // Number of regions supported by the MPU
         using SEPARATE = Field<TYPE_Reg, 0, 1>; // Indicates support for separate instruction and data address maps
    };

    // MPU Control Register
    using CTRL_Reg = Register<MPU_BASE + 0x4, ReadWrite>;
    struct _CTRL {
        using PRIVDEFENA = Field<CTRL_Reg, 2, 1>; // Enables the default memory map as a background region for privileged access
        using HFNMIENA = Field<CTRL_Reg, 1, 1>; // controls whether handlers executing with priority less than 0 access memory
        using ENABLE = Field<CTRL_Reg, 0, 1>; // Enables the MPU
    };

     // MPU Region RNRber Register Selects the region currently accessed by MPU_RBAR and MPU_RASR
    using RNR_Reg = Register<MPU_BASE + 0x8, ReadWrite>;
    struct _RNR {
        using REGION = Field<RNR_Reg, 0, 8>; // Indicates the memory region accessed by MPU_RBAR and MPU_RASR
    };

     // MPU Region Base Address Register
    using RBAR_Reg = Register<MPU_BASE + 0xC, ReadWrite>;
    struct _RBAR {
        using ADDR = Field<RBAR_Reg, 5, 27>; // Base address of the region
        using VALID = Field<RBAR_Reg, 4, 1>; // value specified in this write, and apply the base address update to this region.
        using REGION = Field<RBAR_Reg, 0, 4>; //  can specify the number of the region to update
    };

     // MPU Region Attribute and Size Register
    using RASR_Reg = Register<MPU_BASE + 0x10, ReadWrite>;
    struct _RASR {
        using XN = Field<RASR_Reg, 28, 1>; // Execute Never
        using AP = Field<RASR_Reg, 24, 3>; // Access Permissions
        using TEX = Field<RASR_Reg, 19, 3>; // TEX-level
        using S = Field<RASR_Reg, 18, 1>; // Shareable
        using C = Field<RASR_Reg, 17, 1>; // Cacheable
        using B = Field<RASR_Reg, 16, 1>; // Bufferable
        using SRD = Field<RASR_Reg, 8, 8>; // Subregion Disable
        using SIZE = Field<RASR_Reg, 1, 5>; // Indicates the region size
        using ENABLE = Field<RASR_Reg, 0, 1>; // Enables this region
    };

     // MPU Alias 1 Region Base Address Register
    using RBAR_A1_Reg = Register<MPU_BASE + 0x14, ReadWrite>;
    struct _RBAR_A1 {
        using ADDR = Field<RBAR_A1_Reg, 5, 27>; // Base address of the region
        using VALID = Field<RBAR_A1_Reg, 4, 1>; // value specified in this write, and apply the base address update to this region.
        using REGION = Field<RBAR_A1_Reg, 0, 4>; //  can specify the number of the region to update
    };

       // MPU Alias 1 Region Attribute and Size Register
    using RASR_A1_Reg = Register<MPU_BASE + 0x18, ReadWrite>;
    struct _RASR_A1 {
         using XN = Field<RASR_A1_Reg, 28, 1>; // Execute Never
        using AP = Field<RASR_A1_Reg, 24, 3>; // Access Permissions
        using TEX = Field<RASR_A1_Reg, 19, 3>; // TEX-level
        using S = Field<RASR_A1_Reg, 18, 1>; // Shareable
        using C = Field<RASR_A1_Reg, 17, 1>; // Cacheable
        using B = Field<RASR_A1_Reg, 16, 1>; // Bufferable
        using SRD = Field<RASR_A1_Reg, 8, 8>; // Subregion Disable
        using SIZE = Field<RASR_A1_Reg, 1, 5>; // Indicates the region size
        using ENABLE = Field<RASR_A1_Reg, 0, 1>; // Enables this region
    };

         // MPU Alias 2 Region Base Address Register
    using RBAR_A2_Reg = Register<MPU_BASE + 0x1C, ReadWrite>;
    struct _RBAR_A2 {
        using ADDR = Field<RBAR_A2_Reg, 5, 27>; // Base address of the region
        using VALID = Field<RBAR_A2_Reg, 4, 1>; // value specified in this write, and apply the base address update to this region.
        using REGION = Field<RBAR_A2_Reg, 0, 4>; //  can specify the number of the region to update

    };

       // MPU Alias 2 Region Attribute and Size Register
    using RASR_A2_Reg = Register<MPU_BASE + 0x20, ReadWrite>;
    struct _RASR_A2 {
         using XN = Field<RASR_A2_Reg, 28, 1>; // Execute Never
        using AP = Field<RASR_A2_Reg, 24, 3>; // Access Permissions
        using TEX = Field<RASR_A2_Reg, 19, 3>; // TEX-level
        using S = Field<RASR_A2_Reg, 18, 1>; // Shareable
        using C = Field<RASR_A2_Reg, 17, 1>; // Cacheable
        using B = Field<RASR_A2_Reg, 16, 1>; // Bufferable
        using SRD = Field<RASR_A2_Reg, 8, 8>; // Subregion Disable
        using SIZE = Field<RASR_A2_Reg, 1, 5>; // Indicates the region size
        using ENABLE = Field<RASR_A2_Reg, 0, 1>; // Enables this region
        
    };

          // MPU Alias 3 Region Base Address Register
    using RBAR_A3_Reg = Register<MPU_BASE + 0x24, ReadWrite>;
    struct _RBAR_A3 {
        using ADDR = Field<RBAR_A3_Reg, 5, 27>; // Base address of the region
        using VALID = Field<RBAR_A3_Reg, 4, 1>; // value specified in this write, and apply the base address update to this region.
        using REGION = Field<RBAR_A3_Reg, 0, 4>; //  can specify the number of the region to update
    };

       // MPU Alias 3 Region Attribute and Size Register
    using RASR_A3_Reg = Register<MPU_BASE + 0x28, ReadWrite>;
    struct _RASR_A3 {
         using XN = Field<RASR_A3_Reg, 28, 1>; // Execute Never
        using AP = Field<RASR_A3_Reg, 24, 3>; // Access Permissions
        using TEX = Field<RASR_A3_Reg, 19, 3>; // TEX-level
        using S = Field<RASR_A3_Reg, 18, 1>; // Shareable
        using C = Field<RASR_A3_Reg, 17, 1>; // Cacheable
        using B = Field<RASR_A3_Reg, 16, 1>; // Bufferable
        using SRD = Field<RASR_A3_Reg, 8, 8>; // Subregion Disable
        using SIZE = Field<RASR_A3_Reg, 1, 5>; // Indicates the region size
        using ENABLE = Field<RASR_A3_Reg, 0, 1>; // Enables this region
    };
