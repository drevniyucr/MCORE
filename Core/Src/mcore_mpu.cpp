...
void MPU_Config(void) {
	...
	MPU->RASR =
			((uint32_t) 0x01 << MPU_RASR_XN_Pos) | // istruction access disable
			((uint32_t) 0x00 << MPU_RASR_AP_Pos) | // region no access
			((uint32_t) 0x00 << MPU_RASR_TEX_Pos) | //level 0
			...
			((uint32_t) 0x1F << MPU_RASR_SIZE_Pos) | //size 4GB
